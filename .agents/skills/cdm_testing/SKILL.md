...
| tools | `tests/tools_tests.ch` | yt-dlp/ffmpeg availability + `check_tools_status_json` reporting: `CDM_TOOLS_DIR` redirection, `installed`/`not_installed` status matches reality, status-object structure (`name`/`status`/`version`/`path` + `both_ready`), status agrees with `ytdlp_is_available()`/`ffmpeg_is_available()`; plus regression `CDM_tools_detect_after_install` (binary on disk at canonical path must report `installed`) and `CDM_tools_install_uses_canonical_name` (queueing targets `yt-dlp`, never the renamed `yt-dlp (1)`) | no |
