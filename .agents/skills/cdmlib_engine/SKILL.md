...
JSON, no UI, no settings files, no category routing, no process spawning.
...
start_pending(dm)
  ├─ periodic_save_progress(dm)          // every save_interval_millis while active
...
**Worker-thread rule**: `download_entry` must NOT dereference `job.rt` after
`run_download_task` returns — the manager may detach+delete the runtime the moment the
download finishes. Capture `job.rt.manager` before the call and use only that pointer.
...
Two mutexes, strict ordering — `dm.items_mutex` → `rt.info_mutex`, never reverse.
...
- `detach_runtime(dm, id)` acquires+releases `items_mutex` internally. Call it OUTSIDE
  any `items_mutex` hold (why `cancel_task`/`retry_task`/`shutdown` all detach first).
...
- **Never hold `items_mutex` while calling `join_task()`** — the worker re-enters
  `start_pending` on completion and would deadlock.
...
- `retry_task(id)` — re-queues FAILED/CANCELLED preserving downloaded/total bytes
  (worker resumes from disk), increments `retry_count`, clears `was_interrupted`.
- `restart_task(id)` — deletes the output file + all `.part` files, resets progress to 0,
  re-queues. "Start completely from scratch".
- `resume_task(id)` — PAUSED/QUEUED → QUEUED; FAILED/CANCELLED re-queue with preserved
  progress (like retry but without bumping retry_count).
...
6. **`new T()`, never `malloc(sizeof(T)) as *mut T; *p = T()`** for structs with a
   `mutex`/destructor — the temporary's destructor runs and invalidates the heap copy's
   mutex (`free(): invalid pointer` later).
